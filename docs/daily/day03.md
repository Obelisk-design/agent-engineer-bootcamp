...
| 阶段       | Commit    | 产出                                                                   |
| ---------- | --------- | ---------------------------------------------------------------------- |
| 设计期     | `471469c` | `docs(day03): add streaming design spec`                               |
| 设计期     | `122b5c1` | `docs(day03): spec self-review fixes`                                  |
| 设计期     | `e578a93` | `docs(day03): add streaming implementation plan`                       |
...
## 🪶 YAGNI retrospective：今天主动不做的，留了哪里

Day 03 实施过程中**显式不做**的 5 类能力，都有明确 TODO 锚点。后续 day 不是“补全”，而是“起新 day 设计”。

| 主动不做                          | 为什么不做                                          | TODO 锚点位置                                                                    |
| --------------------------------- | --------------------------------------------------- | -------------------------------------------------------------------------------- |
| `AbortSignal` 取消                | 与 Day 02 `chat()` 对齐；语义需独立 day             | spec §12（开放问题 1）、`chat-client.ts:91` 头注释、`openai-chat-client.ts` TODO |
| 单测 / mock SDK                   | 流式 mock 复杂度高，spec 决策 demo 验证             | spec §2.2、plan Global Constraints、provider 头注释 TODO 段                      |
| HTTP / SSE adapter                | 跨 libs/llm 边界；留给 `apps/api/`                  | spec §8、CLAUDE.md Day 02 §9 宿主原则                                            |
| Vue / 浏览器消费                  | 跨 apps/web-vue 边界；留给未来 day                  | spec §8.2、Day 04 候选 3                                                         |
| 结构化事件 chunk（`tool_use` 等） | 触碰 `AsyncIterable<string>` 契约，需破坏性升级决策 | spec §3.3、§12 开放问题 2、Day 04 候选 2                                         |

> **教学点**：YAGNI 的关键不是“今天不写”，而是“**留了明确 TODO 锚点**”。如果只在心里记下“以后做”，下一次起 day 时需要先考古；写在 spec / 头注释里，新 commit 的 reviewer 也能立刻看到约定来源。
...
**Day 03 实施期带来的额外信号**：`7987bac` 抽出的 `toApiMessages()` 让 Anthropic 请求构造更集中，未来加入 `signal` 选项时不必同时改两份协议适配。这降低了 AbortSignal 的实施成本，但没有降低其设计成本（**signal 应该进入 ChatClient 契约、provider options，还是只属于 `apps/api/` adapter**？—— 思考题 3 是 Day 04 设计阶段必须先答的问题）。

---

## 🔗 相关引用

设计 / 计划 / 实施报告都落在仓库固定路径，未来读者可按这条链一路追到代码：

```text
docs/daily/day03.md                                  ← 本笔记
  └─ docs/superpowers/specs/2026-07-20-chat-client-streaming-design.md
        §2.2   故意不做（YAGNI）
        §3.3   chunk 语义 / 语义等价性边界
        §8     Vue / HTTP / SSE 留 TODO
        §12    开放问题（AbortSignal / 结构化事件 / apps/api / apps/web-vue）

  └─ docs/superpowers/plans/2026-07-20-chat-client-streaming.md
        Global Constraints  抽取今天每个 task 的硬约束
        Task 1～7            完整 brief + 验收门

  └─ .superpowers/sdd/task-N-report.md (N=1..6) + task-4-fix-report.md + task-7-fix-report.md
        每个 task 的实际 run log、typecheck/lint/test 输出、commit SHA
        fix report 记录 review 抓出的真实问题与最小修复

  └─ docs/daily/day02.md
        ChatClient 抽象层 + 多 provider + 宿主原则（Day 03 的前置）
```

代码锚点（line-range 已与 Task 7 fix `9a24918` 同步）：

| 锚点                                                    | 文件                                                                         |
| ------------------------------------------------------- | ---------------------------------------------------------------------------- |
| `chat-client.ts:36-40` —— 接口 `stream()` 契约          | [libs/llm/chat-client.ts](../../libs/llm/chat-client.ts)                     |
| `openai-chat-client.ts:74-78` —— null delta skip        | [libs/llm/openai-chat-client.ts](../../libs/llm/openai-chat-client.ts)       |
| `anthropic-chat-client.ts:99-103` —— 双判别联合         | [libs/llm/anthropic-chat-client.ts](../../libs/llm/anthropic-chat-client.ts) |

...
2. ✅ 修复 OpenAI / Anthropic `chat()` / `stream()` 中 `Message[]` 的 `as unknown` 硬 cast，加入 tool 消息正确映射
3. ✅ 实现最小 `Agent` 类：chat → tool_call → execute → chat 循环，默认 `maxIterations=5`
4. ✅ 落地 `CalculatorTool`：自写 tokenizer + shunting-yard + RPN，不依赖 eval/new Function
5. ✅ 跑通 OpenAI 兼容协议 + Anthropic gateway 的真实 LLM tool calling demo
6. ✅ 补齐 `libs/` 测试（calculator / registry / agent mock）
7. ✅ 守住 YAGNI：不做并行 tool、stream tool calling、AbortSignal、runtime schema validation
...
├── libs/tools/
│   ├── tool.ts                                 # ✏️ 新增 ToolDefinition（事实源）
│   ├── tool-registry.ts                        # ✏️ 移除本地 ToolDefinition，改从 tool.ts 导入
│   ├── calculator-tool.ts                      # 🆕（今日已存在）
│   └── index.ts                                # ✏️ 改 ToolDefinition export 源
├── libs/llm/
│   ├── chat-client.ts                          # ✏️ ToolDefinition 改从 ../tools 导入
│   ├── tool-call.ts                            # ✏️ 移除 ToolDefinition 定义
│   ├── openai-chat-client.ts                   # ✏️ toOpenAIMessages 映射 helper
│   ├── anthropic-chat-client.ts                # ✏️ toApiMessages 扩展 tool_use/tool_result
│   └── index.ts                                # ✏️ ToolDefinition 改从 ../tools 导入
├── libs/agent/
│   ├── agent.ts                                # 🆕 Agent 类 + AgentOptions
...
### 2. `Message` 到 SDK 消息格式：不能靠 `as unknown`
...
|------|--------|-----------|----------|
| 工具声明 | `type: 'function' + function` | `name + description + input_schema` | `chatWithTools` 内部 map |
| assistant tool 调用 | `message.tool_calls[]` | `content[].type==='tool_use'` | `chatWithTools` 解析 |
...
4. `stream()` 今天也做了 tool 消息映射，但 Day 04 没有 `streamWithTools`。如果历史消息里包含 tool 调用，`stream()` 的映射是否正确？会不会有语义问题？
5. `Agent` 的 tool 执行是顺序的。并行执行时，结果消息顺序会怎样影响 LLM 的下一轮决策？OpenAI 与 Anthropic 对并行 tool 结果的支持是否一致？
6. `calculatorTool` 的 `args` 类型是 `{ expression: string }`，但 execute 参数是 `unknown`。这种「运行时信任」在什么 day 应该被 runtime schema validation 替代？引入 zod/ajv 的触发条件是什么？
...
**根因**：`maxIterations=2` 意味着 `chatWithTools` 会被调用 2 次。第二次调用没有 mock 响应，FakeChatClient 先抛错。
...
- [x] `libs/llm/tool-call.ts` 不再定义 `ToolDefinition`
- [x] OpenAI `chat()` / `stream()` / `chatWithTools()` 共享 `toOpenAIMessages`，无 `as unknown` 硬 cast
- [x] Anthropic `chat()` / `stream()` / `chatWithTools()` 共享 `toApiMessages`，无 `as unknown` 硬 cast
...
- [x] `tests/libs/agent/agent.test.ts` 全绿
- [x] `pnpm typecheck` 0 error
- [x] `pnpm lint` 0 error
- [x] `pnpm format:check` 全绿
- [x] `pnpm test` 20 / 20 passed
- [x] Day 02 / Day 03 demos 无回归（chat/stream 契约未动）
...
因此把 `ToolDefinition` 上移到 `libs/tools/tool.ts`，消除双事实源。这是今日唯一与已批准 spec 的设计偏差，属于根因修复（消除重复定义），不是新增能力。
...
1. **AbortSignal 取消**：给 `chatWithTools` 加 `options?: { signal?: AbortSignal }`，让流式/非流式调用都能被取消。

 ...
 * 任一字段缺位时返回 null，由 EmbedDemo 显示红 banner 提示用户补 .env。
 */

import { embed } from '../../../../../libs/embedding/index.js';

// 代码内不再保留任何默认值（base URL / 模型名）。env 缺位时由 getOpenAIConfig 返回 null，
// 由 EmbedDemo 显示红 banner 提示用户补 .env —— 不再"看似能跑 + 偷偷指向内网网关"。
...
export function getOpenAIConfig(): OpenAIConfig {
  ...
  const modelName = typeof modelRaw === 'string' && modelRaw.length > 0 ? modelRaw : null;
...
export async function embedTexts(
  ...
  if (baseUrl === null) throw new RangeError('VITE_OPENAI_BASE_URL not set');
  if (modelName === null) throw new RangeError('VITE_OPENAI_EMBEDDING_MODEL not set');
